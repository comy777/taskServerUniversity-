...
export const saveNote = async (req: Request, res: Response) => {
	...
	if (!lessonValid.state) return res.send({ error: 'La clase no esta registrada' });
...
export const editNote = async (req: Request, res: Response) => {
	...
	if (!noteValid.state) return res.send({ error: 'La nota no esta registrada' });
	if (noteValid.user.toString() !== id) return res.send({ error: 'No tiene permisos para eliminar la nota' });
	try {
