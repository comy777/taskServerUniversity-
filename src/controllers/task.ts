...
export const saveTask = async (req: Request, res: Response) => {
	...
	if (lesson.user.toString() !== user) return res.send({ error: 'No tiene permisos para agregar la tarea' });
	if (!req.body.title && !req.body.body) return res.send({ error: 'Tarea no almacenada' });
	try {
...
export const editTask = async (req: Request, res: Response) => {
	...
	if (taskValid.user.toString() !== user) return res.send({ error: 'No tiene permisos para agregar la tarea' });
	try {
...
export const completeTask = async (req: Request, res: Response) => {
	...
	try {
		...
		await Task.findOneAndUpdate({ _id: id }, { complete: !complete }, { new: true });
